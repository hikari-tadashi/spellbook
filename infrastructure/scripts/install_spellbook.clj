...
(defn write-conf [install-dir ollama-host ollama-model]
  ...
  (let [conf-out (p install-dir "spellbook.conf")]
    (try (require '[create-conf])
         (catch Exception _
           (load-file (str installer-dir "/create_conf.clj"))))
    (spit conf-out ((resolve 'create-conf/build-conf) (str install-dir) ollama-host ollama-model))
